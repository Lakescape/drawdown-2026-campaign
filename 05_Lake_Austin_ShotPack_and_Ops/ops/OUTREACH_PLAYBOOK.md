...
- `campaign:drawdown-2026` — **armed only on LCRA registration opening / COA confirmation**; carries `dd:registered` / `dd:unregistered` sub-state for LCRA Lakewide Permit registration status
...
- **Message angle:** Canal grids trap sediment — the Island canals and Oyster Landing basin shoal visibly at 492.13 ft and will be mudflats in a -10 ft drawdown; bulkheads on these 1970s–80s canals are at end-of-life; <25 cy/address City of Austin administrative approval covers routine slip cleanouts, and a registered drawdown unlocks up to 2,000 cy under the LCRA/USACE Lakewide Permit. Lead with a free slip-depth + bulkhead condition survey (sounding pole + photo report).
- **Cadence:**
...
- **Message angle:** Vendor-packet play: COI + W-9 + license/insurance + Lake Austin permit fluency (COA dock registration, HLDO Tier I/II, drawdown Lakewide Permit, TPWD/LCRA aquatic-treatment approvals) + references from their own communities (only with client permission). Offer a no-cost common-area shoreline condition report (their park/day-dock/marina) as the door-opener — that's a deliverable the manager can hand their board.
- **Cadence:**
...
- **Trigger (all required):** (a) City of Austin/LCRA formally announce a Lake Austin drawdown (fall-2026 window under study as of 2026-07-27; Jan–Feb window being petitioned by Friends of Lake Austin), AND (b) LCRA opens registration at lcra.org/lakelowerings for the USACE Lakewide Permit. Until both, only `campaign:drawdown-watch` nurture runs (educational, no deadline claims — **never** claim a drawdown date before confirmation).
...
- **Message angle:** The drawdown is the once-a-decade window: -10 ft exposes lakebed for maintenance dredging up to **2,000 cy per registered address** (vs <25 cy normal), wall/slip/ramp repair under the Lakewide Permit, site-plan exemption for ≤25 cy cleanouts, and land-based equipment replacing barge day-rates. Deadline pressure is real and legal **only because** the window is agency-announced — cite the announcement in every piece.

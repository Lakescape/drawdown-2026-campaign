...
BEATS = [
    ("v1", "f730acf1ac49", "bleed", 3.30, "in",
     "HOOK — looking straight down off a dock deck into weed-choked water. "
     "The dock IS in frame, so the card does not overclaim (QC swap 2026-08-31)."),
    ("v2", "924c9e39682b", "bleed", 4.35, "out",
     "STAKES — clear water, a hydrilla mound you only see when the lake drops."),
...
CARDS = [
    ...
    ("v3", [("TWO TRUXORS.", 78, CREAM),
            ("PROJECTED 10–12 FT.", 60, COPPER)], 0.74),
...
for slot, prefix, mode, dur, kb, why in BEATS:
    ...
    if mode == "bleed":
        ...
        px, py = 0, int(CH * 0.44) - fg.height // 2
        out.paste(fg, (px, py))
...
for i, (slot, *_) in enumerate(BEATS):
    ...
    parts.append(f"[{ci}:v]format=rgba,{fades}[c{i}]")
    parts.append(f"[{cur}][c{i}]overlay=0:0:enable='between(t,{starts[i]:.2f},"
                 f"{starts[i]+BEATS[i][3]:.2f})'[o{i}]")
    cur = f"o{i}"

fg = ";".join(parts)
# No anullsrc. This used to mux a silent AAC stream, which measured -70.0 LUFS
# and read as a broken audio track in QC. Truly silent (-an) matches every other
# cut in this folder; a VO pass adds a real track later.
cmd = ["ffmpeg", "-y", *ins,
       "-filter_complex", fg,
       "-map", f"[{cur}]",
       "-t", f"{TOTAL:.2f}",
